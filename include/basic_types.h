...
/* strncpy() was removed from newer kernel headers (superseded by strscpy());
 * this reimplements classic strncpy() semantics (zero-pad remainder, no
 * guaranteed NUL-termination) for source compatibility with this driver. */
#ifdef __KERNEL__
static inline char *rtw_strncpy(char *dst, const char *src, size_t n)
{
	size_t i;
...
#define strncpy(dst, src, n) rtw_strncpy(dst, src, n)
#endif
